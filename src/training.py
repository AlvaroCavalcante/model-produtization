...
long_tail_variables = ['anos_desde_criacao', 'instagram_num', 'facebook_num',
                       'min_camp', 'interacoes_g1', 'tempo_desperd', 'iteracao_volei', 'iteracao_atletismo']
...
def ceil(x): return 1 if x >= 1 else x
...
def roc_pr_curve(y, y_pred_prob, show=True):
    ...
    ax.scatter([], [], label='AUC: {:.02f}'.format(
        roc_auc_score(y, y_pred_prob)), color='#F76800')
    ...
    ax.scatter([], [], label='Random: {:.02f}'.format(
        auc(rs_recall, rs_precision)), color='#333333')
    ax.scatter([], [], label='Model: {:.02f}'.format(
        auc(ms_recall, ms_precision)), color='#F76800')
